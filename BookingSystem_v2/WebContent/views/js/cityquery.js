...
$(document).ready(function(){ //fromcity与tocity对应不同的效果，引用input的ID需要与页面中的相一致
	$('#city1').querycity({'data':cityResource,'tabs':labelFromcity,'hotList':hotList});
	$('#city2').querycity({'data':cityResource,'tabs':labelFromcity,'hotList':hotList});
	$('#city3').querycity({'data':cityResource,'tabs':labelFromcity,'hotList':hotList});
	$('#city4').querycity({'data':cityResource,'tabs':labelFromcity,'hotList':hotList});
...
(function($){
    $.querycity = function(input,options){
        ...
        if($.trim(input.val())=='' || $.trim(input.val())==options.defaultText){ 
            input.val(options.defaultText).css('color','#717071');
        }
        var t_pop_focus = false;
        var t_suggest_focus = false;
        var t_suggest_page_click = false;
		//#cityselector用于在页面中的目标定位
        $('#cityselector').append("<div id='pop_city_"+input.attr('id')+"' class='pop_city' style='display:none;'><p class='pop_head'></p><ul class='list_label'></ul><div class='pop_city_container'></div></div>");
        $('#cityselector').append("<div id='suggest_city_"+input.attr('id')+"' class='list_city' style='display:none;'><div class='list_city_head'></div><div class='list_city_container'></div><div class='page_break'></div></div>");
        ...
        input.focus(function(){
		    ...
		    if($.trim($(this).val())==options.defaultText){
			    $(this).val('').css('color','#717071'); //input内容
	    	}
		    popMain.show();
        }).click(function(){		
            if(t_suggest_page_click){
...
;
		    if(isHave){
	    		...
	    		suggestMain.find('.list_city_head').html("<span class='msg'>暂无匹配城市："+value+"</span>");
		    }
            suggestMain.show();
	    }
        ...
        function setAddPageHtml(pageIndex){
            ...
            if(suggestMain.find('.list_city_container').children().length > options.suggestLength){
                ...
                if(pageIndex > 1){
                    cityPageBreak.append("<a href='javascript:void(0)' inum='"+(pageIndex-1)+"'>上页</a>");
                }	
			    ...
			    if (pageIndex<pageBreakSize) {
                    cityPageBreak.append("<a href='javascript:void(0);' inum='"+ (i) +"'>下页</a>");
                }
                cityPageBreak.show();           
            }else{
                cityPageBreak.hide();    
    ...
    $.fn.querycity = function(options){
        var defaults = {
            ...
            'hotList'       : '',            
            'defaultText'   : '中文或拼音',
            'popTitleText'  : '输入中文或拼音',
            'suggestTitleText' : '输入中文或拼音，可用↑↓选择',
            'suggestLength' : 10,
            'pageLength'    : 5, 
            'onSelect'      : '' 
        };
