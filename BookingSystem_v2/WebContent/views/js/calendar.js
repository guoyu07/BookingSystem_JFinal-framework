...
//mvAry[mvAry.length]  = '  <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套，改用 Div
  ...
  mvAry[mvAry.length]  = '        <th align="left" width="1%"><input style="font-size:12px; border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;" name="prevMonth" type="button" id="prevMonth" value="&lt;" /></th>';
  ...
  for(var i = 0; i < 6;i++){
    ...
    for(var j = 0; j < 7; j++) {
      if (j == 0){
        ...
        mvAry[mvAry.length]  = '  <td style="font-size:12px; cursor:default;"></td>';
      }
    }
    mvAry[mvAry.length]  = '    </tr>';
  }
...
Calendar.prototype.show = function (dateObj, popControl) {
  ...
  this.panel.style.left = xy.x +0 + "px";
...
document.write('<div id="ContainerPanel" style="display:none;"><div id="calendarPanel" style="position:absolute; display:none; z-index:99;');
document.write('background-color:#efefef; border:1px solid #ccc; width:202px; margin-left:1px;"></div>');
if(document.all)
{
document.write('<iframe style="position:absolute;z-index:-1;width:expression(this.previousSibling.offsetWidth);');
